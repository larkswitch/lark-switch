...
### 2026-09-20：静态标记相同仍可能有影子凭据

Windows 的写时复制视图可以继承宿主标记，同时只隔离实际 token 值。因此标记相同不是宿主执行的充分条件。0.2.13 起，普通 Windows 桌面启动先转交任务调度器；普通 shim 和 lpcctl 经桌面桥接执行。直接访问凭据只允许自举后的桌面，以及该桌面直接派生的受管子进程。`LPC_HOST_EXECUTOR_PID` 只是查验提示，必须同时核对实际父进程、安装路径和自举参数，不能靠设置环境变量绕过。

桥接协议同步传递目标程序、工作目录和显式标准输入；协议不匹配或宿主不可用时禁止退回当前凭据视图。桌面和 sidecar 必须一起升级。`host CLI bridge request completed` 记录目标与退出码，不记录业务参数。账号体检保留脱敏 CLI 诊断，诊断报告不再把“配置文件存在但需要登录”判为通过。
